...
def decode(u):
    ...
    try:
        if u.startswith("https://nam12.safelinks.protection.outlook.com"): 
            u = u.split("url=")[1]
            u = u.split("&data=")[0]
            u = unquote(u)

        url = urldefense_decoder.decode(u)
        return url
    except ValueError as e:
        print(e)
        try:
            return u.encode('idna').decode()
        except:
            return u
...
def main():
    ...
    print("What is the url you wish to decode?")
    u = input(">")
    urldefense_decoder = URLDefenseDecoder()
    try:
        print("")
        print(urldefense_decoder.decode(u))
        print("")
    except ValueError as e:
        print(e)
